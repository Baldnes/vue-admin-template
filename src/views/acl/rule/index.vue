<template>
  <el-card style="height: 80px;">
    <el-form :inline="true" class="form">
      ...
      <el-form-item>
        <el-button type="success" :disabled="keyword ? false : true" @click="search">搜索</el-button>
  ...
  <el-card style="margin: 10px 0;">
    ...
    <el-table style="margin: 10px 0;" border :data="allRole">
      ...
      <el-table-column label="操作" width="300" align="center">
        <template #="{row}">
          <el-button type="success" size="small" icon="User" @click="setPermisstion(row)">分配权限</el-button>
...
<!--    分配职位-->
    <el-drawer v-model="drawer">
      ...
      <template #default>
        <span>注：按钮太多，只做了用户管理与品牌管理的添加权限，如拿到代码想完善按钮权限（请在每个页面上的按钮添加全局自定义指令v-has="`后端返回按钮标识`"）</span>
        <div>
          <el-tree
              :data="menuArr"
              show-checkbox
...
import {reqAddOrUpdateRole, reqAllMenuList, reqAllRoleList, reqRemoveRole, reqSetPermisstion} from "@/api/acl/rule";
import type {RoleResponseData, Records, RoleData, MenuResponseData,MenuList} from "@/api/acl/rule/type.ts";
